"""
Live API Verification Script for MedLens AI Agent
...
def test_live_suite():
    print("=" * 60)
    print("MEDLENS AI AGENT LIVE API VERIFICATION")
    print("=" * 60)
    ...
    health_data = r.json()
    print(f"   [SUCCESS] Status: {health_data['status']}, Service: {health_data['service']}")
    assert "MedLens" in health_data["service"], f"Expected MedLens in service, got {health_data['service']}"
    ...
    assert "MedLens" in res_c["proposed_action"]["message"]
    ...
    print("\n" + "=" * 60)
    print("ALL LIVE REST API TESTS PASSED SUCCESSFULLY FOR MEDLENS!")
    print("=" * 60)
